...
await test('auto-reply persists configuration and only answers eligible DMs', async () => {
  ...
  Math.random = () => 0;
  try {
    assert.equal(await maybeAutoReply(api as never, 'default', 'group-thread', ThreadType.Group), false);
    assert.equal(await maybeAutoReply(api as never, 'default', 'dm-thread', ThreadType.User), true);
    assert.equal(await maybeAutoReply(api as never, 'default', 'dm-thread', ThreadType.User), false, 'peer cooldown');
    assert.equal(sends, 1);
    const syntheticTgId = sentMsgStore.getByZaloMsgId('default', 'auto-reply-zalo-id');
    assert.equal(typeof syntheticTgId, 'number');
    assert.ok(syntheticTgId! < 0);
    assert.equal(sentMsgStore.isSendingTo('default', 'dm-thread'), false);
  } finally {
...
test('auto-reply releases the in-flight marker after a failed send', async () => {
  ...
  try {
    ...
    } as never, 'default', 'dm-failure', ThreadType.User);
    assert.equal(failed, false);
    assert.equal(sentMsgStore.isSendingTo('default', 'dm-failure'), false);
  } finally {
    globalThis.setTimeout = originalSetTimeout;
    setAutoReplyEnabled(false);
  }
