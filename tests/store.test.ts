...
test('topic store keeps topic and Zalo indexes one-to-one', () => {
  store.set({ accountId: 'default', topicId: 9101, zaloId: 'za', type: 0, name: 'A' });
  store.set({ accountId: 'default', topicId: 9101, zaloId: 'zb', type: 0, name: 'B' });
  assert.equal(store.getTopicByZalo('default', 'za', 0), undefined);
  assert.equal(store.getTopicByZalo('default', 'zb', 0), 9101);

  store.set({ accountId: 'default', topicId: 9102, zaloId: 'zb', type: 0, name: 'B2' });
...
test('topic store updates names and removes both directions', () => {
  store.set({ accountId: 'default', topicId: 9103, zaloId: 'group-x', type: 1, name: 'Old' });
  ...
  assert.equal(store.getTopicByZalo('default', 'group-x', 1), undefined);
});
...
test('msgStore ignores sentinel IDs and deduplicates input IDs', () => {
  ...
  msgStore.save('default', 9201, ['0', '', 'm-9201', 'm-9201'], quote('m-9201'));
  const after = msgStore.stats();
  assert.equal(msgStore.getTgMsgId('default', '0'), undefined);
  assert.equal(msgStore.getTgMsgId('default', 'm-9201'), 9201);
...
test('msgStore remapping moves reference ownership and removes orphan quote', () => {
  msgStore.save('default', 9202, ['shared-remap'], quote('old'));
  msgStore.save('default', 9203, ['shared-remap'], quote('new'));
  assert.equal(msgStore.getTgMsgId('default', 'shared-remap'), 9203);
...
test('msgStore patches echo metadata without losing thread identity', () => {
  msgStore.save('default', 9204, ['echo-id'], quote('placeholder', 'group-echo', 1));
...
test('sentMsgStore clears stale reverse IDs when an entry is replaced', () => {
  sentMsgStore.save(9301, { accountId: 'default', msgIds: ['z-old'], zaloId: 'peer', threadType: 0 });
  sentMsgStore.save(9301, { accountId: 'default', msgIds: ['z-new'], zaloId: 'peer', threadType: 0 });
  assert.equal(sentMsgStore.getByZaloMsgId('default', 'z-old'), undefined);
  assert.equal(sentMsgStore.getByZaloMsgId('default', 'z-new'), 9301);
});

test('sentMsgStore reference-counts concurrent in-flight sends', () => {
  sentMsgStore.markSending('default', 'peer-concurrent');
  sentMsgStore.markSending('default', 'peer-concurrent');
  sentMsgStore.unmarkSending('default', 'peer-concurrent');
  assert.equal(sentMsgStore.isSendingTo('default', 'peer-concurrent'), true);
  sentMsgStore.unmarkSending('default', 'peer-concurrent');
  assert.equal(sentMsgStore.isSendingTo('default', 'peer-concurrent'), false);
});
...
test('debounced msgStore persistence writes a gzip payload', async () => {
  msgStore.save('default', 9601, ['persist-id'], quote('persist-id'));
