...
export const store = {
  ...
  set(entry: TopicEntry): void {
    ...
    if (previousAtTopic) {
      const previousKey = zaloKey(previousAtTopic.accountId, previousAtTopic.zaloId, previousAtTopic.type);
      if (previousKey !== zaloKey(entry.accountId, entry.zaloId, entry.type)
...
export const msgStore = {
  ...
  save(accountId: string, tgMsgId: number, zaloMsgIds: string[], quote: ZaloQuoteData): void {
    ...
    const newIdCount = validIds.reduce((count, rawId) => {
      const id = `${accountId}_${rawId}`;
      return count + (_zaloToTg.has(id) ? 0 : 1);
    }, 0);
...
export const sentMsgStore = {
  ...
  save(tgMsgId: number, info: SentMsgInfo): void {
    ...
    if (previous) {
      const prevAccId = previous.accountId || 'default';
      for (const mid of previous.msgIds) {
        const key = `${prevAccId}_${mid}`;
  ...
  markSending(accountId: string, zaloId: string): void {
    const key = `${accountId}_${zaloId}`;
    const current = _pendingSendConvos.get(key);
    _pendingSendConvos.set(key, {
  ...
  unmarkSending(accountId: string, zaloId: string): void {
    ...
    if (!current || current.count <= 1) {
      _pendingSendConvos.delete(key);
      return;
    }
    _pendingSendConvos.set(key, { ...current, count: current.count - 1 });
  },
  ...
  isSendingTo(accountId: string, zaloId: string): boolean {
    const key = `${accountId}_${zaloId}`;
    const pending = _pendingSendConvos.get(key);
...
export interface ReactionTargetInfo {
  zMsgId: string;
  actorId: string;
}

export const reactionEventDedupeStore = {
  add(accountId: string, tgMsgId: number, targetInfo: ReactionTargetInfo, reactionEmoji: string, originalActorName: string): void {
    const accountZaloKey = `${accountId}_${targetInfo.zMsgId}`;
    const key = `${accountZaloKey}_${targetInfo.actorId}_${reactionEmoji}`;
  },
  
  addOutbound(accountId: string, tgMsgId: number, reactionEmoji: string, count: number, _debugActorInfo: string): void {
    const key = `${accountId}_${tgMsgId}_${reactionEmoji}`;
  },
